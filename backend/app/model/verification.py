...
class Verification(Base):
    ...
    # the EXPECTED animal, resolved from the scanned QR token
    ...
    similarity_score: Mapped[float] = mapped_column(Float, nullable=False)  # cosine similarity vs. template
    ...
    quality_flags: Mapped[Optional[dict]] = mapped_column(JSONB, nullable=True)  # e.g. {"blur": false}
    ...
    muzzle_template: Mapped["MuzzleTemplate"] = relationship(back_populates="verifications")

    media_asset: Mapped["MediaAsset"] = relationship()
    verified_by_user: Mapped[Optional["User"]] = relationship(back_populates="verifications_performed")
